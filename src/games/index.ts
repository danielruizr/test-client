...
const parseGamePayload = (
  payload?: GamePayload
): (Game & WithBlockchains) | null => {
  if (!payload) {
    return null;
  }
  return {
...
): Promise<(Game & WithBlockchains) | null> => {
...
const createGameBlockchains = async (
  gameId: string,
...
export const setGameBlockchains = async (
  gameId: string,
  blockchains: string[]
) => {
...
export const setGameGenres = async (
  gameId: string,
  genres: GameGenreEnum[]
) => {
  await deleteGameGenres(gameId);
  if (genres.length) await createGameGenres(gameId, genres);
};

export const setGamePlatforms = async (
  gameId: string,
  platforms: GamePlatformEnum[]
) => {
...
export const listGameAssets = async (
  gameId: string,
  forceRefresh: boolean = false
) => {
  const { data } = await hasuraService.query<{ payload: GameAsset[] }>(
    LIST_GAME_ASSETS,
    { gameId },
    { forceRefresh }
  );
...
export const createGameGenres = async (
  gameId: string,
  genres: GameGenreEnum[],
) => {
...
export const createGamePlatforms = async (
  gameId: string,
  platforms: GamePlatformEnum[],
) => {
  const { data } = await hasuraService.mutate<{ payload: { affected_rows: number} }>(
