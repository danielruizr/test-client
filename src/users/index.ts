...
export const getUser = async (id: string, forceRefresh: boolean = false) => {
  const { data } = await hasuraService.query<{ payload: User }>(
    GET_USER,
    { id },
    { forceRefresh }
  );
...
export const getUserWithWallets = async (id: string, forceRefresh: boolean = false) => {
  const { data } = await hasuraService.query<{ payload: UserWithWallet }>(
...
export const updateUser = async (
  data: Partial<
    Pick<User, 'username' | 'profilePicPath' | 'bannerImgPath' | 'bio'>
  >
) => {
  const { data: res } = await hasuraService.mutate<{ payload: Partial<User> }>(
    UPDATE_USER,
    { data }
  );
  return res!.payload;
