...
export const getPost = async (addresses: string[], forceRefresh = false) => {
    const result = await Promise.all(
        addresses.map(projectAddress => hasuraService.query<{ mirror: { entries: any[] } }>(GET_ENTRIES, { projectAddress }, { forceRefresh }))
    );
    return result
        .map(
            ({
...
export const getDigestsByAddress = async (projectAddress: string, forceRefresh = false) => {
    ...
    } = await hasuraService.query<{ mirror: { entries: { originalDigest: string; publisher: { project: { address: string } } }[] } }>(
...
export const getPostByDigest = async (digest: string, forceRefresh = false) => {
    ...
    } = await hasuraService.query<{ mirror: { entry: any } }>(GET_ENTRY_BY_DIGEST, { digest }, { forceRefresh });
    return entry;
};
