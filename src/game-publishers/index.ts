...
export const createGamePublisher = async (gamePublisher: Partial<GamePublisher>) => {
...
export const updateGamePublisher = async (id: string, input: Partial<GamePublisher>) => {
    const { data } = await hasuraService.mutate<{ payload: GamePublisher }>(
...
export const listGamePublishers = async (forceRefresh: boolean = false) => {
    const { data } = await hasuraService.query<{ payload: GamePublisher[] }>(LIST_GAME_PUBLISHERS, undefined, { forceRefresh });
